 ...
 */
    ...
    void *my_sbrk(intptr_t increment) {
        ...
        //    the arena, return -1.
        if (_brk - (char *) arena > arenaSize - increment)
            return (void *) -1;
...
int main(int argc, char const *argv[]) {
    ...
    if ((fd = open("/Users/mano233/Documents/c_projects/untitled3/map", O_RDWR)) < 0) {
        perror("open");
    }
    ...
    arena = mmap(nullptr, sb.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

    ((char *) arena)[0] = '0';
    if (my_sbrk(sb.st_size) == (void *) -1)
        return -1;
    if ((msync((void *) arena, sb.st_size, MS_SYNC)) == -1) {
        perror("msync");
    }

    if ((munmap((void *) arena, sb.st_size)) == -1) {
        perror("munmap");
    }
    close(fd);
}
