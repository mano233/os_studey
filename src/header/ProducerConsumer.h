//
// Created by mano233 on 2020/10/30.
//

#ifndef UNTITLED3_PRODUCERCONSUMER_H
#define UNTITLED3_PRODUCERCONSUMER_H
...
static const int kItemRepositorySize = 20; // Item buffer size.
static const int kItemsToProduce = 1000;   // How many items we plan to produce.
std::mutex my_mutex;//多线程标准输出同步锁
...
void test() {
    ...
    std::thread consumer2(ConsumerTask); // 创建消费之线程.
    std::thread consumer3(ConsumerTask); // 创建消费之线程.
    std::thread consumer4(ConsumerTask); // 创建消费之线程.
    producer.join();
    consumer.join();
    consumer2.join();
    consumer3.join();
...
#endif //UNTITLED3_PRODUCERCONSUMER_H
